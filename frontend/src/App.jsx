...
import { checkBackend } from './lib/tts';
...
// Check TTS backend once on app load so Header/Landing can show "Natural voice" when available
...
function LibraryView() {
  ...
  const normalizeImportUrl = useCallback((raw) => {
    let u;
  ...
  const handleImportUrl = useCallback(async () => {
    const input = window.prompt('Paste a file URL (direct link, Google Drive, Dropbox, GitHub raw):');
    if (!input) return;
    const url = normalizeImportUrl(input.trim());
    try {
      addToast({ type: 'info', title: 'Importing...', message: 'Fetching file from URL...' });
      ...
      const guessedName = decodeURIComponent(pathname.split('/').pop() || '').trim() || `import-${Date.now()}.pdf`;
      ...
      addToast({
        ...
        message: err?.message || 'Could not import from this URL. Try a direct downloadable link.',
      });
    }
  }, [addToast, handleFile, normalizeImportUrl]);
