...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen w-full max-w-[100vw] overflow-x-hidden bg-brand-charcoal font-sans antialiased selection:bg-brand selection:text-white">
      {/* Navigation */}
      <nav className="fixed top-0 left-0 right-0 w-full max-w-[100vw] z-50 glass-effect safe-area-top" data-purpose="main-navigation">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 sm:h-20 flex items-center justify-between gap-2 min-w-0">
          <Logo className="text-white shrink-0 min-w-0" />
          {/* Desktop nav */}
          <div className="hidden sm:flex items-center gap-2 sm:gap-4 text-sm font-medium text-slate-400">
            ...
            <button type="button" onClick={goToLibrary} className="px-3 sm:px-5 py-1.5 sm:py-2.5 bg-primary hover:bg-primary-hover text-white rounded-lg font-semibold transition-all shadow-lg shadow-primary/20 text-sm sm:text-base">
              Go to Library
            </button>
          </div>
          {/* Mobile: menu button */}
          <div className="flex sm:hidden items-center gap-2">
            <button type="button" onClick={goToLibrary} className="px-3 py-2 bg-primary hover:bg-primary-hover text-white rounded-lg font-semibold text-sm">
              Library
            </button>
            <button type="button" onClick={() => setMobileMenuOpen((v) => !v)} className="text-slate-400 hover:text-white p-2.5 min-h-[44px] min-w-[44px] flex items-center justify-center rounded-lg" data-purpose="mobile-menu-toggle" aria-label={mobileMenuOpen ? 'Close menu' : 'Open menu'} aria-expanded={mobileMenuOpen}>
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">{mobileMenuOpen ? <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /> : <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16m-7 6h7" />}</svg>
            </button>
          </div>
        </div>
        {/* Mobile menu overlay */}
        {mobileMenuOpen && (
          <div className="sm:hidden fixed inset-x-0 top-[3.5rem] bottom-0 z-40 bg-brand-charcoal/98 backdrop-blur-md border-t border-white/10 overflow-y-auto safe-area-bottom" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
            <div className="p-6 space-y-6">
              <a href="#features" onClick={() => setMobileMenuOpen(false)} className="block py-3 text-slate-300 hover:text-white font-medium">
                Features
              </a>
              {isPiperReady() && (
                <div className="flex items-center gap-2 py-3 text-emerald-400 text-sm font-medium">
                  <span className="material-symbols-outlined">check_circle</span>
                  Natural voice ready
                </div>
              )}
              <button type="button" onClick={() => { goToLibrary(); setMobileMenuOpen(false); }} className="w-full py-4 bg-primary hover:bg-primary-hover text-white rounded-xl font-bold text-base">
                Go to Library
              </button>
            </div>
          </div>
